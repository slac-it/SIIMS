...
namespace SIIMS
{
    public partial class rep_expIssueCode : System.Web.UI.Page
    {
        ...
        private void gridBinding(string iCode_ID)
        {
            ...
            string _sql = " select issue.issue_id, issue.title, issue.description, org.name as org,p.name as owner, issue.SIG_LEVEL, sta.status, to_char( issue.created_on,'MM/DD/YYYY') as Date_Created,to_char( issue.last_on,'MM/DD/YYYY') as Date_LastModified " +
                ...
                " FROM siims_issue issue join siims_org org on issue.org_id=org.org_id  " +
                ...
                " WHERE issue.is_active='Y' and exists (select * from siims_issue_code where issue_id=issue.issue_id and is_worktype='Y') " +
